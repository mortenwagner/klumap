...
export default function Welcome() {
  ...
  const [showingStepper, setShowingStepper] = useState(false)
  ...
  return (
    <div className="flex-1 flex flex-col lg:flex-row min-h-screen">
      ...
      <div className="flex-1 flex items-center justify-center px-6 sm:px-10 py-10 lg:py-0">
        <div className="max-w-md w-full space-y-8">
          {showingStepper ? (
            <OnboardingStepper
              onComplete={() => {
                dispatch({ type: 'SET_GUIDED_ONBOARDING', value: true })
                dispatch({ type: 'NEXT_SCREEN' })
              }}
              onSkip={() => dispatch({ type: 'NEXT_SCREEN' })}
            />
          ) : (
            <>
              {/* O-Ring motif */}
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <button
                  onClick={() => setShowingStepper(true)}
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                ...
                <button
                  ...
                  className="group flex-1 bg-white/5 border border-white/10 rounded-xl p-3 text-left cursor-pointer hover:bg-white/[0.08] hover:border-white/20 transition-all"
                >
                  ...
                  <span className="inline-flex items-center gap-1 mt-2 text-[11px] text-white/25 group-hover:text-white/50 transition-colors">
                    Learn more
      ...
      <div className={`flex-1 flex items-center justify-center px-6 sm:px-10 py-10 lg:py-0 lg:border-l border-white/[0.04] ${showingStepper ? 'hidden lg:flex' : ''}`}>
