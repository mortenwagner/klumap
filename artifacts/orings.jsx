...
function FeedbackArrow({ from, to, delay = 0, visible }) {
  // from/to are ring radii — arrow curves from the "from" ring inward to the "to" ring
  // Arrow sits on the left side of the diagram
  const fromR = from / 2;
  const toR = to / 2;
  const midR = (fromR + toR) / 2;

  // Start point: left side of the "from" ring, slightly above center
  const startX = CTR - fromR;
  const startY = CTR - 15;

  // End point: left side of the "to" ring, slightly below center
  const endX = CTR - toR;
  const endY = CTR + 15;

  // Control point: bulge outward to the left
  ...
  // Arrowhead points
  const aSize = 10;
  // Arrow points inward-right and slightly down
...
export default function ORingsOfInnovation({ lang = "en" }) {
  ...
  // Step 0: only red core. Step 1: blue ring appears. Step 2: grey ring appears.
  ...
  // Dim earlier rings slightly when a new one builds
